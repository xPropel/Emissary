...
window.update = () ->
  ...
  # Reconstruct Query String from Options
  query = {}
 
  for key in [ "map", "mode", "player", "spec"]
    if $("##{key}s").prop("selectedIndex") in [1..$("##{key}s").prop("length")-1]
      URI.addQuery(query, key, $("##{key}s").prop("selectedIndex"))
 
  if $("#lobbyName").val() then URI.addQuery(query, "lobby", $("#lobbyName").val())
  if $("#lobbyPass").val() then URI.addQuery(query, "pass",  $("#lobbyPass").val())
 
  if $("#number").val() then URI.addQuery(query, "codes", $("#number").val())
 
  for key in [ "rpass", "rhash", "index" ]
    if $("##{key}").prop("checked")
      URI.addQuery(query, key, "1")
 
  window.history.replaceState(query, "", "?#{URI.buildQuery(query)}");
